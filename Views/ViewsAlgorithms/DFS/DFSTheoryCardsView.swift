...
struct DFSTheoryCardsView: View {
    ...
    // LOAD DFS DATA
    let cards: [TheoryCard] = TheoryData.dfsCards
        ...
        ZStack {
            ...
            .onAppear {
                if currentCardID == nil {
                    let savedIndex = UserProgressManager.shared.getLastCardIndex(moduleId: "dfs")
            ...
            .onChange(of: currentCardID) { oldValue, newValue in
                if let id = newValue, let index = cards.firstIndex(where: { $0.id == id }) {
                    UserProgressManager.shared.updateLastCardIndex(moduleId: "dfs", cardIndex: index)
                }
            }
        }
        .toolbar {
            ...
            ToolbarItem(placement: .topBarTrailing) {
                ...
                .accessibilityHint("Double tap to open the interactive DFS visualizer.")
            }
        }
    ...
    var body: some View {
        VStack(spacing: 40) {
            ...
            Image(systemName: card.iconName)
                ...
                .foregroundStyle(card.isBridge ? .teal : .cyan)
            ...
            if card.isBridge {
                NavigationLink(destination: DFSQuizView()) {
                    HStack {
                        Text("Take the Quiz")
                            .font(.headline)
